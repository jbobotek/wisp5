...
/** @todo write the comments for 4 below so I can actually read them... */ //ok, I can comment this if needed
...
// RFID TIMINGS (Taken a bit more liberately to support both R420 and R1000). //critical timing numbers
...
//TIMING----------------------------------------------------------------------------------------------------------------------------//
//Goal is 56.125/62.500/68.875 us/micro seconds. Trying to shoot for the lower to save (a little) power.
...
#define NUM_QUERY_BITS  (22)    //comment what bits mean here?
...
typedef struct {							//define these things!
    uint8_t     TRext;                      /** @todo What is this member? Relates to Interrogator -> Tag communication*/
    uint16_t    handle;                     /** @todo What is this member? */
    uint16_t    slotCount;                  /**Corresponds to a point in an inventory round (think your number in a DMV queue).
     	 	 	 	 	 	 	 	 	 	Device/TAG replies when its slotCount reaches 0*/

    uint8_t     Q;                          /**Q is a pseudo random number generated by the TAG/WISP for the Interrogator.
    										Loaded into slot counter. Int range of 0-15. Interrogator uses Q to construct probability
    										of TAG response (2^Q) Used in random slotted collision allocation, i.e. when there are
    										multiple TAGs*/
...
//FUNCTION PROTOTYPES---------------------------------------------------------------------------------------------------------------//
extern void WISP_doRFID(void);//what is this for? - Jeremy
...
#define BITTOG(port,pin)    port ^= (pin) //reminder, ^ is XOR
